...
EMA (指数移动平均)、SMA (简单移动平均)、RSI (相对强弱指标)、MACD (移动平均收敛散度)、ADX (平均趋向指数)、STOCH (随机指标)、CCI (顺势指标)、WMA (加权移动平均)、TEMA (三重指数移动平均)、Ichimoku (一目均衡表)
...
2. 运行回测测试策略效果: `freqtrade backtesting -s conny`
