...
本策略是一个**EMA金叉/死叉、RSI超买超卖、布林带突破、MACD交叉、趋势判断**，旨在通过技术指标分析市场趋势，寻找买入和卖出机会。

### 核心逻辑

使用EMA快慢线交叉判断趋势转折点，金叉买入，死叉卖出；使用RSI指标判断市场是否处于超买或超卖状态；使用布林带判断价格是否突破支撑或阻力位；使用MACD快慢线交叉判断趋势方向；使用移动平均线判断市场整体趋势方向
...
EMA (指数移动平均)、SMA (简单移动平均)、RSI (相对强弱指标)、MACD (移动平均收敛散度)、ADX (平均趋向指数)、TEMA (三重指数移动平均)、SAR (抛物线指标)
...
- 最小ROI设置为 120001.50000000001%，过于激进或不现实
...
2. 运行回测测试策略效果: `freqtrade backtesting -s PRICEFOLLOWINGX`
