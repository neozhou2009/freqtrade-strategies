...
本策略是一个**EMA金叉/死叉、RSI超买超卖、MACD交叉、成交量确认、趋势判断**，旨在通过技术指标分析市场趋势，寻找买入和卖出机会。

### 核心逻辑

使用EMA快慢线交叉判断趋势转折点，金叉买入，死叉卖出；使用RSI指标判断市场是否处于超买或超卖状态；使用MACD快慢线交叉判断趋势方向；结合成交量验证价格信号的可靠性；使用移动平均线判断市场整体趋势方向
...
SMA (简单移动平均)、RSI (相对强弱指标)、MACD (移动平均收敛散度)、SAR (抛物线指标)
...
### 1. 导入方式问题
1. 使用了旧版接口版本 INTERFACE_VERSION = 2

### 2. 修复措施
1. 已更新为 INTERFACE_VERSION = 3 以获得最新功能和兼容性

## 投资逻辑问题分析

本策略在投资逻辑和风险管理方面存在以下问题：

- 最小ROI设置为 1440001.0%，过于激进或不现实
...
2. 运行回测测试策略效果: `freqtrade backtesting -s Strategy005`
