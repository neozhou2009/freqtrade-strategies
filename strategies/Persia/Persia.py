...
timeframes = (
    COSTUMETF  # if COSTUMETFENABLED else np.delete(tf_arr,np.argwhere(tf_arr < 2))
)
...
class Persia(IStrategy):
    ...
    timeframe0 = (
        CategoricalParameter(
            timeframes, default=timeframes[0], optimize=0 < CONDITIONS, space="buy"
        )
        if COSTUMETFENABLED
        else IntParameter(
            timeframes[0],
    ...
    sell_timeframe0 = (
        CategoricalParameter(
            timeframes, default=timeframes[0], optimize=0 < CONDITIONS, space="sell"
        )
        if COSTUMETFENABLED
        else IntParameter(
            timeframes[0],
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        for i in range(CONDITIONS):
            ...
            crossed_timeframe_idx = (
                f"{getattr(self, 'sell_crossed_timeframe' + i).value}"
            )
