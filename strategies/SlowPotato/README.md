...
- **时间框架**: 5m
- **止损设置**: -0.99
- **最小ROI**: #if you overide this it will sell once it reaches a certain ROI threshold rather than the sell logic
        "0":  99

## 策略意图和目的

本策略是一个**成交量确认、趋势判断**，旨在通过技术指标分析市场趋势，寻找买入和卖出机会。

### 核心逻辑

结合成交量验证价格信号的可靠性；使用移动平均线判断市场整体趋势方向
...
2. 运行回测测试策略效果: `freqtrade backtesting -s SlowPotato`
