...
class CombinedBinHAndClucV5Hyperoptable(IStrategy):
    ...
    buy_bin_bbdelta_close = RealParameter(
        0.004, 0.15, default=0.008, space="buy", optimize=True, load=True
    )
    buy_bin_closedelta_close = RealParameter(
        0.01, 0.03, default=0.0175, space="buy", optimize=True, load=True
    )
    buy_bin_tail_bbdelta = RealParameter(
        0.1, 0.5, default=0.25, space="buy", optimize=True, load=True
    )
    buy_cluc_close_bblowerband = RealParameter(
        0.5, 1.5, default=0.985, space="buy", optimize=True, load=True
    )
    buy_cluc_volume = IntParameter(
        15, 30, default=20, space="buy", optimize=True, load=True
    )
    ...
    def custom_stoploss(
        ...
        **kwargs,
        ...
        if (current_profit < 0) & (
            current_time - timedelta(minutes=300) > trade.open_date_utc
        ):
            return 0.01
        return 0.99
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        dataframe.loc[
            (  # strategy BinHV45
                ...
                & (
                    dataframe["tail"]
                    < (dataframe["bbdelta"] * self.buy_bin_tail_bbdelta.value)
                )
    ...
    class HyperOpt:
        ...
        @staticmethod
        def sell_indicator_space() -> List[object]:
            return []
