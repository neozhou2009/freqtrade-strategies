...
本策略是一个**EMA金叉/死叉、MACD交叉、成交量确认、趋势判断**，旨在通过技术指标分析市场趋势，寻找买入和卖出机会。

### 核心逻辑

使用EMA快慢线交叉判断趋势转折点，金叉买入，死叉卖出；使用MACD快慢线交叉判断趋势方向；结合成交量验证价格信号的可靠性；使用移动平均线判断市场整体趋势方向
...
2. 运行回测测试策略效果: `freqtrade backtesting -s stratfib`
