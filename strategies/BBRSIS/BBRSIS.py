...
class BBRSIS(IStrategy):
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe_short = resample_to_interval(
            dataframe, self.get_ticker_indicator() * 3
        )
        dataframe_medium = resample_to_interval(
            dataframe, self.get_ticker_indicator() * 6
        )
        dataframe_long = resample_to_interval(
            dataframe, self.get_ticker_indicator() * 10
        )
        ...
        dataframe.loc[
            (
                ...
                & (
                    ...
                    < (
                        dataframe[
                            "resample_{}_rsi".format(self.get_ticker_indicator() * 3)
                        ]
                        - 5
                    )
                )
        ...
        dataframe.loc[
            (
                ...
                & (
                    dataframe["rsi"]
                    > dataframe[
                        "resample_{}_rsi".format(self.get_ticker_indicator() * 6)
                    ]
                )
                & (
                    dataframe["rsi"]
                    > dataframe[
                        "resample_{}_rsi".format(self.get_ticker_indicator() * 10)
                    ]
                )
                & (dataframe["volume"] > 0)
            ),
