...
EMA (指数移动平均)、SMA (简单移动平均)、RSI (相对强弱指标)、MACD (移动平均收敛散度)、ATR (平均真实波幅)、ADX (平均趋向指数)、STOCH (随机指标)、CCI (顺势指标)、TEMA (三重指数移动平均)、SAR (抛物线指标)、MFI (资金流量指标)、ROC (变动率指标)
...
### 1. 导入方式问题
1. 使用了已废弃的方法名 `populate_buy_trend` 和 `populate_sell_trend`
2. 使用了旧版接口版本 INTERFACE_VERSION = 2

### 2. 修复措施
1. 已更新为 `populate_entry_trend` 和 `populate_exit_trend`
