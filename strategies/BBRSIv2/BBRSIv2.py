...
class BBRSIv2(IStrategy):
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe["dropped_by_percent"] = 1 - (
            dataframe["close"] / dataframe["close_max"]
        )
        dataframe["close_min"] = dataframe["close"].rolling(window=60).min()  # 5h
        dataframe["pumped_by_percent"] = (
            dataframe["high"] - dataframe["close_min"]
        ) / dataframe["high"]
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if conditions:
            dataframe.loc[
                # can_sell &
                # is_live_data &
                reduce(lambda x, y: x | y, conditions), "exit"
            ] = 1
