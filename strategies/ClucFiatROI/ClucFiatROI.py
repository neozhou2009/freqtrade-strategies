...
class ClucFiatROI(IStrategy):
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if not active_trade:
            conditions.append(
                ...
                & (
                    (
                        dataframe["bb1-delta"].gt(
                            dataframe["close"] * params["bbdelta-close"]
                        )
                        & dataframe["closedelta"].gt(
                            dataframe["close"] * params["closedelta-close"]
                        )
                        & dataframe["tail"].lt(
                            dataframe["bb1-delta"] * params["bbdelta-tail"]
                        )
                    ...
                    | (
                        ...
                        & (
                            dataframe["close"]
                            < params["close-bblower"] * dataframe["lower-bb2"]
                        )
                        & (
                            dataframe["volume"]
                            < (
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe.loc[
            (
                (dataframe["close"] * params["sell-bbmiddle-close"])
                > dataframe["mid-bb2"]
            )
