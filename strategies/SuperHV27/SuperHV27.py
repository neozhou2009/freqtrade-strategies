...
class SuperHV27(IStrategy):
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if trade_data["active_trade"]:
            ...
            conditions.append(
                ...
                > (trade_data["peak_profit"] * profit_factor)
            )
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if trade_data["active_trade"]:
            loss_cutoff = self.linear_growth(
                -0.03, 0, 0, 300, trade_data["open_minutes"]
            )
    ...
    def min_roi_reached_entry(
        ...
        if dynamic_roi and dynamic_roi["enabled"]:
            ...
            if dynamic_roi["type"] == "linear":
                rate = (dynamic_roi["start"] - dynamic_roi["end"]) / dynamic_roi[
                    "decay-time"
                ]
                min_roi = max(
                    dynamic_roi["end"], dynamic_roi["start"] - (rate * trade_dur)
                )
            # exponential decay: f(t) = start * e^(-rate*t)
                ...
                min_roi = max(
                    dynamic_roi["end"],
                    dynamic_roi["start"]
                    * np.exp(-dynamic_roi["decay-rate"] * trade_dur),
                )
            elif dynamic_roi["type"] == "connect":
                # connect the points in the defined table with lines
                past_roi = list(
                    filter(lambda x: x <= trade_dur, self.minimal_roi.keys())
                )
                next_roi = list(
                    filter(lambda x: x > trade_dur, self.minimal_roi.keys())
                )
    ...
    def populate_trades(self, pair: str) -> dict:
        ...
        if self.config["runmode"].value in ("live", "dry_run"):
            ...
            if active_trade:
                ...
                trade_data["current_profit"] = active_trade[0].calc_profit_ratio(
                    current_rate
                )
                ...
                trade_data["open_candles"]: int = (
                    open_minutes // active_trade[0].timeframe
                )
            else:
    ...
    def check_entry_timeout(
        self, pair: str, trade: Trade, order: dict, **kwargs
    ) -> bool:
