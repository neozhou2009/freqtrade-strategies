...
class TrixV23Strategy(IStrategy):
    ...
    buy_trix_timeperiod = IntParameter(
        5, 25, default=9, space="buy", optimize=False, load=True
    )
    ...
    buy_rsi_timeperiod = IntParameter(
    ...
    buy_ema_enabled = BooleanParameter(
        default=False, space="buy", optimize=False, load=True
    )
    buy_ema_timeperiod = IntParameter(
        9, 100, default=21, space="buy", optimize=False, load=True
    )
    ...
    buy_btc_ema_enabled = BooleanParameter(
        default=False, space="buy", optimize=True, load=True
    )
    ...
    sell_trix_timeperiod = IntParameter(
        5, 25, default=9, space="sell", optimize=False, load=True
    )
    ...
    sell_rsi_timeperiod = IntParameter(
    ...
    sell_atr_enabled = BooleanParameter(
        default=True, space="sell", optimize=False, load=True
    )
    sell_atr_timeperiod = IntParameter(
        9, 30, default=14, space="sell", optimize=False, load=True
    )
    ...
    def custom_stoploss(
        ...
        **kwargs,
        ...
        stoploss_price = (
            last_candle["low"]
            - last_candle[f"atr_{self.sell_atr_timeperiod.value}"]
            * self.sell_atr_multiplier.value
        )
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        for val in self.buy_trix_timeperiod.range:
            dataframe[f"trix_b_{val}"] = ta.EMA(
                ta.EMA(
                    ta.EMA(dataframe[self.buy_trix_src.value], timeperiod=val),
                    timeperiod=val,
                ),
                timeperiod=val,
            )
        ...
        for val in self.buy_trix_signal_timeperiod.range:
            dataframe[f"trix_b_signal_{val}"] = ta.SMA(
                dataframe["trix_b_pct"], timeperiod=val
            )

        for val in self.sell_trix_timeperiod.range:
            dataframe[f"trix_s_{val}"] = ta.EMA(
                ta.EMA(
                    ta.EMA(dataframe[self.sell_trix_src.value], timeperiod=val),
                    timeperiod=val,
                ),
                timeperiod=val,
            )
        ...
        for val in self.sell_trix_signal_timeperiod.range:
            dataframe[f"trix_s_signal_{val}"] = ta.SMA(
                dataframe["trix_s_pct"], timeperiod=val
            )
        ...
        for val in self.buy_stoch_rsi_timeperiod.range:
            dataframe["b_stoch_rsi"] = (
                ...
                dataframe["b_rsi"].rolling(val).max()
                - dataframe["b_rsi"].rolling(val).min()
            )
        ...
        for val in self.sell_stoch_rsi_timeperiod.range:
            dataframe["s_stoch_rsi"] = (
                ...
                dataframe["s_rsi"].rolling(val).max()
                - dataframe["s_rsi"].rolling(val).min()
            )

        # EMA
        for val in self.buy_ema_timeperiod.range:
            dataframe[f"ema_b_{val}"] = ta.EMA(
                dataframe[self.buy_ema_src.value], timeperiod=val
            )
        ...
        dataframe["stoploss_price"] = (
            dataframe["low"]
            - dataframe[f"atr_{self.sell_atr_timeperiod.value}"]
            * self.sell_atr_multiplier.value
        )
