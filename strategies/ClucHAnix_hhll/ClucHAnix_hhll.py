...
class ClucHAnix_hhll(IStrategy):
    ...
    rocr_1h = RealParameter(
        0.5, 1.0, default=0.54904, space="buy", optimize=is_optimize_clucHA
    )
    bbdelta_close = RealParameter(
        0.0005, 0.02, default=0.01965, space="buy", optimize=is_optimize_clucHA
    )
    closedelta_close = RealParameter(
        0.0005, 0.02, default=0.00556, space="buy", optimize=is_optimize_clucHA
    )
    bbdelta_tail = RealParameter(
        0.7, 1.0, default=0.95089, space="buy", optimize=is_optimize_clucHA
    )
    close_bblower = RealParameter(
        0.0005, 0.02, default=0.00799, space="buy", optimize=is_optimize_clucHA
    )

    is_optimize_hh_ll = False
    buy_hh_diff_48 = DecimalParameter(
        0.0, 15, default=1.087, optimize=is_optimize_hh_ll
    )
    buy_ll_diff_48 = DecimalParameter(
        -23, 40, default=1.087, optimize=is_optimize_hh_ll
    )
    ...
    sell_fisher = RealParameter(
        0.1, 0.5, default=0.38414, space="sell", optimize=is_optimize_sell
    )
    sell_bbmiddle_close = RealParameter(
        0.97, 1.1, default=1.07634, space="sell", optimize=is_optimize_sell
    )
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        informative["sma_200_dec_20"] = informative["sma_200"] < informative[
            "sma_200"
        ].shift(20)
                ...
                (
                    ...
                    & (
                        dataframe["bbdelta"].gt(
                            dataframe["ha_close"] * self.bbdelta_close.value
                        )
                    )
                    & (
                    ...
                    & (
                        dataframe["tail"].lt(
                            dataframe["bbdelta"] * self.bbdelta_tail.value
                        )
                    )
        ...
        dataframe.loc[
            (
                ...
                | (
                    ...
                    & (
                        dataframe["close"]
                        > (dataframe["ema_24"] * self.high_offset_2.value)
                    )
                    & (dataframe["rsi"] > 50)
                ...
                | (
                    ...
                    & (
                        dataframe["close"]
                        > (dataframe["ema_24"] * self.high_offset.value)
                    )
                    & (dataframe["rsi_fast"] > dataframe["rsi_slow"])
                )
            )
