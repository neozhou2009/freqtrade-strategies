...
class NowoIchimoku1hV1(IStrategy):
    ...
    def populate_indicators(self, df: DataFrame, metadata: dict) -> DataFrame:
        df["upper"] = bollinger_bands(
            df["close"], moving_average="hma", length=20, mult=2.5
        )["upper"]
        ...
        df["upper_cloud"] = df["lead_1"].where(
            df["lead_1"] > df["lead_2"], df["lead_2"]
        )
        df["lower_cloud"] = df["lead_1"].where(
            df["lead_1"] < df["lead_2"], df["lead_2"]
        )
