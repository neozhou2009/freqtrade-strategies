...
class fahmibah(IStrategy):
    ...
    clucha_enabled = BooleanParameter(
        default=buy_params["clucha_enabled"], space="buy", optimize=False
    )
    rocr_1h = DecimalParameter(
        0.5, 1.0, default=0.54904, space="buy", decimals=5, optimize=False
    )
    bbdelta_close = DecimalParameter(
        0.0005, 0.02, default=0.01965, space="buy", decimals=5, optimize=False
    )
    closedelta_close = DecimalParameter(
        0.0005, 0.02, default=0.00556, space="buy", decimals=5, optimize=False
    )
    bbdelta_tail = DecimalParameter(
        0.7, 1.0, default=0.95089, space="buy", decimals=5, optimize=False
    )
    close_bblower = DecimalParameter(
        0.0005, 0.02, default=0.00799, space="buy", decimals=5, optimize=False
    )

    # buy params lambo1
    lambo1_enabled = BooleanParameter(
        default=buy_params["lambo1_enabled"], space="buy", optimize=False
    )
    lambo1_ema_14_factor = DecimalParameter(
        0.5, 2.0, default=1.054, space="buy", decimals=3, optimize=True
    )
    lambo1_rsi_4_limit = IntParameter(
        0, 50, default=buy_params["lambo1_rsi_4_limit"], space="buy", optimize=True
    )
    lambo1_rsi_14_limit = IntParameter(
        0, 50, default=buy_params["lambo1_rsi_14_limit"], space="buy", optimize=True
    )

    # hard stoploss profit
    pHSL = DecimalParameter(
        -0.500, -0.040, default=-0.08, decimals=3, space="sell", load=True
    )
