...
> **策略编号**: #12 (465 个策略中的第 12 个)  
> **策略类型**: 多指标动量策略（KAMA + RMI + MACD）  
> **时间框架**: 5 分钟 (5m)
...
**Kamaflage** 是一个复杂的多指标动量策略，结合了 KAMA（考夫曼自适应移动平均）、RMI（相对动量指标）、MACD 等多个技术指标，并引入了订单簿（orderbook）检查机制来优化订单执行。策略名称"Kamaflage"可能来源于"KAMA"和"Camouflage"（伪装）的组合。
...
minimal_roi = {
    "0": 0.15,      # 立即退出：15% 利润
    "10": 0.10,     # 10 分钟后：10% 利润
    "20": 0.05,     # 20 分钟后：5% 利润
    "30": 0.025,    # 30 分钟后：2.5% 利润
    "60": 0.01,     # 60 分钟后：1% 利润
}
...
# 追踪止损
trailing_stop = True
trailing_stop_positive = 0.01125       # 1.125% 追踪启动
trailing_stop_positive_offset = 0.04673  # 4.673% 偏移触发
trailing_only_offset_is_reached = True
```

**设计思路**：
- **多级 ROI**：5 级递减 ROI，持仓时间越长退出门槛越低
- **追踪止损**：4.67% 利润后启动 1.125% 追踪，适合趋势行情
...
conditions = [
    ...
    dataframe["macd"] > params["macd"],                   # MACD > 阈值 (0)
    dataframe["macdhist"] > params["macdhist"],           # MACD 柱 > 阈值 (0)
    dataframe["rmi"] > dataframe["rmi"].shift(),          # RMI 上升
    dataframe["rmi"] > params["rmi"],                     # RMI > 阈值 (50)
...
- **MACD 金叉**：MACD 线在信号线上方且大于 0，动量向上
- **MACD 柱确认**：MACD 柱大于 0，动量增强
- **RMI 动量**：RMI 指标上升且高于 50
...
**工作机制**：
1. 利润达到 4.673% 后启动追踪止损
2. 从最高点回撤 1.125% 时触发退出
...
Kamaflage 是一个多指标动量策略，基于"多条件确认"的核心哲学。
...
**Kamaflage** 是一个设计精良的多指标动量策略，它的核心价值在于：
