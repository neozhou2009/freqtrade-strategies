...
class AlexNexusForgeV8AIV3(IStrategy):
    ...
    stoploss_max_reasonable = DecimalParameter(-0.30, -0.15, default=-0.05, space="sell", optimize=True)
    ...
    def populate_indicators(self, dataframe: pd.DataFrame, metadata: dict) -> pd.DataFrame:
        ...
        dataframe["ema200"] = ta.EMA(dataframe["close"], timeperiod=200) # V3: Macro Trend
    ...
    def populate_entry_trend(self, dataframe: pd.DataFrame, metadata: dict) -> pd.DataFrame:
        ...
        # === V3 FIX: MACRO TREND FILTER ===
        # Filter counter-trend trades to improve win rate and reduce drawdown
        if 'ema200' in dataframe.columns:
            # 1. Filter Longs: Only trade LONG if price is ABOVE EMA200
            dataframe.loc[
                (dataframe['close'] < dataframe['ema200']),
                'enter_long'] = 0
                
            # 2. Filter Shorts: Only trade SHORT if price is BELOW EMA200
            dataframe.loc[
                (dataframe['close'] > dataframe['ema200']),
                'enter_short'] = 0

        # === V3 FIX: REDUCE SHORT BIAS ===
        # Additional safety for shorts in bull markets (RSI check)
        dataframe.loc[
            (dataframe['rsi'] > 50), # Don't enter short if RSI > 50 (momentum is bullish)
            'enter_short'] = 0
