...
graph TD
    ...
    subgraph Phase1 ["核心准备阶段 - Local/CI"]
        ...
        A -->|"prepare_test_env.py"| C["test/user_data/ 策略工作区平铺"]
...
- **机制**：由子目录构建的项目管理方式不利于引擎挂载，此模块负责平铺所有活动策略并复制至一级的 `test/user_data/strategies/` 下。同时映射通用的基准配置文件及行情数据包至隔离执行区，杜绝对原代码库产生污染。
